...
<header class=" w-full z-999 <?= $paginaPrincipal ? 'fixed top-0 left-0' : 'sticky top-0' ?>">
  <div id="divHeader" class="flex justify-between items-center p-5 transition-colors duration-300 ease-in-out  font-bold <?= $paginaPrincipal ? 'text-white bg-transparent' : 'bg-[#5C3B1E] text-[#FFE08A]' ?>">
    <div>
      <a href="index.php">  
      <img src="logo.png" alt="Logo" class="w-16"></a>
    </div>

    <!-- Botón de menú móvil (solo visible en móvil) -->
    <button id="mobile-menu-button" class="md:hidden focus:outline-none cursor-pointer">
          ...
          if($_SESSION["tipo"]!="a"){
            ?>
              <a href="index.php?action=verPerfil" class="mr-5">
                <span class="w-12 h-12 border-2 border-[#FFE08A] rounded-full flex items-center justify-center transition-all duration-300 hover:shadow-md">
...
<div id="mobile-menu" class="fixed top-16 left-0 right-0 hidden md:hidden bg-[#5C3B1E] text-[#FFE08A] w-full px-5 pb-5 transform transition-all duration-300 ease-in-out origin-top scale-y-0">
