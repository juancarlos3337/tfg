...
class usuario{
    ...
    public function comprobarUsuario($nombre,$pw,$email){
        $sentencia="select count(*) from usuarios where nombre=? and pw=? and email=?;";
        $consulta= $this->con->prepare($sentencia);
        $consulta->bind_param("sss",$nombre,$pw,$email);
